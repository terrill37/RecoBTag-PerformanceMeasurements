...
parser.add_option("-d","--dir",dest="jobsDir",type="str",default="Jobs",help="job Directory",metavar="FLAVOUR")
...
cleaningoutputjobs = "rm -rf "+ opts.jobsDir
makingoutputjobs = "mkdir "+ opts.jobsDir
#make directories for the jobs
try:
#    os.system('rm -rf Jobs')
#    os.system('mkdir Jobs')

    os.system(cleaningoutputjobs)
    os.system(makingoutputjobs)
except:
    print "err!"
    pass
...
for i in range(0, nJobs):
    ...
    jobDir = MYDIR+'/'+opts.jobsDir+'/Job_%s/'%str(i)
    ...
    tmp_job.write("cp JetTree_mc.root %s/JetTree_mc_%s.root\n"%(remoteDir, str(i)))
    tmp_job.write("rm hltoutput_hlt.root\n")
    tmp_job.write("rm JetTree_mc.root\n")
    tmp_job.close()
    ...
    tmp_cfgFile = open(jobDir+'/hlt_tmp.py','w')
...
condor_str += "request_memory = 4 GB\n"
